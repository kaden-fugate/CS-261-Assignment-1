...
struct dynarray* dynarray_create()
{
    ...
    new_arr->size = 0, new_arr->capacity = 2;
    new_arr->data = malloc(new_arr->capacity * sizeof(void*));
...
void dynarray_insert(struct dynarray* da, void* val)
{
    ...
    if(da->size == da->capacity){ // execute when at capacity

        da->capacity = da->capacity * 2; // double capacity
        da->data = realloc(da->data, da->capacity * sizeof(void*));

    }

    da->data[da->size] = val; // size = index + 1 so we can use size to add new element to end
    da->size++;
...
{
    /* start at index of removed element, iterate while making
    cur element the next element */
...
{
    *(da->data + idx) = val; // data at index points to val
}
