...
void list_free(struct list* list)
{
    ...
    while(list->head){ // iterate through list

        temp = list->head; // node were deleting
...
void list_insert(struct list* list, void* val)
{
    ...
    list->head = malloc(sizeof(struct node)); // create new head and point next to next vals
    list->head->val = val, list->head->next = temp_node; 
...
void list_insert_end(struct list* list, void* val)
{
    ...
    struct node *new = malloc( sizeof( struct node ) ); // initialize new node
    new->val = val, new->next = NULL;
   
    if(!list->head){ // conditions if list is empty
    ...
    while(temp->next){ // iterate until temp points to final node
    
        temp = temp->next;
    
    }

    temp->next = new; // next points to new
...
void list_remove(struct list* list, void* val, int (*cmp)(void* a, void* b))
{
    ...
    if( (*cmp)(cur->val, val) == 0 ){ // if head is val
    ...
    while(cur){ // iterate through loop until first instance or NULL

        prev = cur, cur = cur->next; // increment prev and cur

        if( (*cmp)(cur->val, val) == 0 ){
            
            prev->next = cur->next; // if same, skip cur node and free cur
            free(cur);
            return;
...
{
    if(!list->head->next){ free(list->head); list->head = NULL; return; } // if list is one long, free without extra work
	
    struct node *cur = list->head, *prev = NULL;

    while(cur->next){ // iterate until cur points to end of list
    ...
    prev->next = NULL; // unlink cur, delete it
    free(cur);
...
int list_position(struct list* list, void* val, int (*cmp)(void* a, void* b))
{
    struct node *temp_node = list->head;
    int idx = 0;
    while(temp_node){ // iterate until match or NULL

        if( (*cmp)(temp_node->val, val) == 0 ){ return idx; } // if true, return idx
        temp_node = temp_node->next, idx++;
    
    }

    return -1; // returns -1 when loop breaks
}
...
void list_reverse(struct list* list)
{
    ...
    while(cur){ // iterate until NULL

        next = cur->next; // store node vals starting at nth place

        cur->next = prev; // list points to prev vals
        prev = cur; // prev is now prev with one new reversed val at head pos

        cur = next; // cur is now non-reversed list at nth place
